...
export default async function Commissions() {
  ...
  const Chart = () => <div className="card" style={{marginTop:16}}>
    <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between'}}>
      ...
      <div style={{fontSize:12.5, color:'#73736c'}}>
        {monthName(thisPeriod)} so far <b style={{color:'#1a1a18', fontVariantNumeric:'tabular-nums'}}>{usd(curM.earned)}</b>
        {delta !== null && <span style={{color: delta >= 0 ? '#2e6b3f' : '#9a551a', fontWeight:700}}> · {delta >= 0 ? '+' : ''}{delta}% vs {lastM.label}</span>}
      </div>
    </div>
    <div style={{display:'flex', alignItems:'flex-end', gap:6, height:110, marginTop:14}}>
      {series.map(s => <div key={s.key} title={monthName(s.key) + ' · ' + usd(s.earned)}
        ...
        <div style={{width:'100%', maxWidth:34, height: Math.max(3, Math.round(88 * s.earned / maxE)),
          borderRadius:2, background: s.current ? '#2257c5' : s.earned > 0 ? '#1a1a18' : '#e3e3dd'}}/>
        <span style={{fontSize:9.5, color: s.current ? '#2257c5' : '#73736c', fontWeight: s.current ? 700 : 500,
  ...
  const Ledger = ({ name, p, showPayout }) => {
    ...
    return <>{Object.entries(periods).sort((a, b) => b[0].localeCompare(a[0])).map(([period, rs]) => {
      ...
      return <div key={period} style={{marginTop:14, ...(isCurrent ? {background:'#f5f9ff', borderRadius:2, padding:'10px 12px', margin:'14px -12px 0'} : {})}}>
        <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:6}}>
          <span className="cardtitle" style={{marginBottom:0, ...(isCurrent ? {color:'#2257c5'} : {})}}>
          ...
          {unpaid.length === 0 ? <span className="pill green" style={{fontSize:10, fontWeight:700}}>PAID OUT</span>
            : closed ? (showPayout
                ...
                : <span className="pill" style={{background:'#f3e8d5', color:'#9a551a', fontSize:10, fontWeight:700}}>PAYS {payoutDateStr.toUpperCase()}</span>)
            : null}
        </div>
        {rs.map(r => {
          const i = inv[r.invoice_id];
          return <div key={r.id} style={{display:'flex', gap:12, alignItems:'center', padding:'6px 0',
            borderBottom:'1px solid ' + (isCurrent ? '#e4eefb' : '#f2f2ee'), fontSize:12.5}}>
            <span style={{color:'#73736c', width:78}}>{new Date(r.created_at).toLocaleDateString()}</span>
            <span style={{flex:1, minWidth:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
              {i ? <>№{String(i.invoice_number).padStart(4,'0')} · {i.collectors ? [i.collectors.first_name, i.collectors.last_name].filter(Boolean).join(' ') : ''} <span style={{color:'#73736c'}}>· {i.title}</span></> : 'Payment'}</span>
            <span style={{color:'#73736c'}}>{Number(r.pct) > 0 ? r.pct + '%'
              : <span className="pill" style={{background:'#f3e8d5', color:'#9a551a', fontSize:9.5, fontWeight:700}}>RATE PENDING</span>}</span>
            <span style={{fontWeight:700, fontVariantNumeric:'tabular-nums',
              color: Number(r.amount_cents) > 0 ? '#2e6b3f' : '#c2c2bb'}}>+{usd(r.amount_cents)}</span>
          </div>; })}
      </div>; })}</>;
  };
  ...
  return <Shell active="commissions">
    ...
    <div className="stats">
      ...
      <div className="stat"><div className="n" style={{color: sum(payable) > 0 ? '#9a551a' : undefined}}>{usd(sum(payable))}</div>
    ...
    {personal
      ? <div className="card" style={{marginTop:16}}>
          ...
          {!(people[viewer]?.rows || []).length && <div style={{fontSize:13, color:'#73736c'}}>
      ...
      : <>
          ...
          {Object.entries(people).sort((a, b) => sum(b[1].rows.filter(r => !r.paid_at)) - sum(a[1].rows.filter(r => !r.paid_at))).map(([name, p]) => {
            ...
            return <details key={name} className="edit" style={{padding:0, overflow:'hidden'}}>
              <summary style={{padding:'13px 18px', display:'grid',
                ...
                <span>{rule ? <span className="pill" style={{background:'#eeeee9', fontWeight:700, fontSize:10.5,
                  display:'inline-flex', height:24, alignItems:'center'}}>{rule.pct}% RATE</span>
                  : <span className="pill" style={{background:'#f3e8d5', color:'#9a551a', fontSize:10, fontWeight:700,
                      display:'inline-flex', height:24, alignItems:'center'}}>NO RATE SET</span>}</span>
                <span style={{textAlign:'right'}}>
                  <span style={{fontVariantNumeric:'tabular-nums', fontWeight:600, color: pAcc ? '#2257c5' : undefined}}>{pAcc ? usd(pAcc) : '—'}</span>
                  <span style={{display:'block', fontSize:11, color:'#73736c'}}>earning now</span></span>
                <span style={{textAlign:'right'}}>
                  <span style={{fontVariantNumeric:'tabular-nums', fontWeight:700, color: pPay ? '#9a551a' : undefined}}>{pPay ? usd(pPay) : '—'}</span>
                  <span style={{display:'block', fontSize:11, color:'#73736c'}}>payable {payoutDateStr}</span></span>
                <span style={{textAlign:'right'}}>
                  <span style={{fontVariantNumeric:'tabular-nums', fontWeight:600}}>{pPaid ? usd(pPaid) : '—'}</span>
                  <span style={{display:'block', fontSize:11, color:'#73736c'}}>paid {now.getFullYear()}</span></span>
              </summary>
              <div style={{padding:'4px 18px 16px', borderTop:'1px solid #eeeee9'}}>
                <Ledger name={name} p={p} showPayout={true}/>
                {!p.rows.length && <div style={{fontSize:12.5, color:'#73736c', marginTop:10}}>
        ...
        <details className="edit" style={{marginTop:16}}>
          ...
          <div style={{fontSize:12, color:'#73736c', marginTop:6}}>
